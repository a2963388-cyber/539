...
def gen_gc(strategies):
    ...
    覆蓋度高→低取前 10 碼，同票取小號。
    10 碼基準（49選6）：均中期望 1.224 碼/期、中≥3碼 9.0%；碼數與 8 碼
    策略不同，滿 50 期評估時只與自身隨機基準比較，不與 8 碼策略直接比。
...
def update_html(new_draws: list, dry_run: bool = False):
    ...
    new_pending = {'strategies': new_strategies, 'excluded': calc_exclude_zone(all_records), 'ts': int(time.time() * 1000)}
